/**
  ******************************************************************************
  * @file    model.cc
  * @author  Pierre-Emmanuel Novac <penovac@unice.fr>, LEAT, CNRS, Université Côte d'Azur, France
  * @version 1.0.0
  * @date    24 march 2020
  * @brief   Template generating plain C code for the implementation of Convolutional Neural Networks on MCU
  */

#ifndef SINGLE_FILE
#include "number.h"
#include "model.h"

 // InputLayer is excluded
#include "conv1d_22.c"
#include "weights/conv1d_22.c" // InputLayer is excluded
#include "max_pooling1d_17.c" // InputLayer is excluded
#include "flatten_8.c" // InputLayer is excluded
#include "dense_9.c"
#include "weights/dense_9.c"
#endif

void cnn(
  const number_t input[MODEL_INPUT_CHANNELS][MODEL_INPUT_SAMPLES],
  dense_9_output_type dense_9_output) {

  // Output array allocation
  static union {
    conv1d_22_output_type conv1d_22_output;
  } activations1;

  static union {
    max_pooling1d_17_output_type max_pooling1d_17_output;
    flatten_8_output_type flatten_8_output;
  } activations2;


  //static union {
//
//    static input_9_output_type input_9_output;
//
//    static conv1d_22_output_type conv1d_22_output;
//
//    static max_pooling1d_17_output_type max_pooling1d_17_output;
//
//    static flatten_8_output_type flatten_8_output;
//
  //} activations;

  // Model layers call chain
 // InputLayer is excluded 
  conv1d_22(
     // First layer uses input passed as model parameter
    input,
    conv1d_22_kernel,
    conv1d_22_bias,
    activations1.conv1d_22_output
  );
 // InputLayer is excluded 
  max_pooling1d_17(
    
    activations1.conv1d_22_output,
    activations2.max_pooling1d_17_output
  );
 // InputLayer is excluded 
  flatten_8(
    
    activations2.max_pooling1d_17_output,
    activations2.flatten_8_output
  );
 // InputLayer is excluded 
  dense_9(
    
    activations2.flatten_8_output,
    dense_9_kernel,
    dense_9_bias, // Last layer uses output passed as model parameter
    dense_9_output
  );

}
